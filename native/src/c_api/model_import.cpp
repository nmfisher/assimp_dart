...
namespace assimp_dart
{

    extern "C"
    {

        // This package is always built with Assimp linked in (no compile-time
        // opt-in), so support is a constant true.
    ...
    struct ModelImporterData {
                                                                     ...
                                                                     const char* extensionHint)
        {
            ...
            // NOTE: thermion's version of this function called
            // Assimp::DefaultLogger::create(...)/kill() around the parse
            // "for debugging". That logger is PROCESS-GLOBAL state, and this
            // library may be entered from several Dart isolates (threads) at
            // once: concurrent create/kill pairs race on the singleton and
            // corrupt the heap ("double free or corruption", reproduced with
            // tool/stress_concurrent_import.dart). The logger also attached
            // no stream, so it never produced output — errors come from
            // importer->GetErrorString() below, which needs no logger. It is
            // therefore not created at all.
            try
            {
                ...
                //
                // NOTE: aiProcess_FlipUVs is intentionally NOT applied here. UV flipping
                // is handled on the Dart side (RawMesh.flipUVs), so applying it in
                // Assimp too would double-flip.
                unsigned int flags =
