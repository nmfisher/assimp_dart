...
// This package is always built with Assimp enabled, so this always returns
// true; it is kept for API parity with the thermion-side facade.
//
// Only FBX export is compiled into the libassimp shipped in the Filament
// artifacts (see thermion's scripts/patch_libassimp_tnt.py, which enables the
// FBX exporter sources and disables every other exporter).
